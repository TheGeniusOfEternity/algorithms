import { createAsyncGetter, Item, RenderFn } from './common';
import { renderItems as noAsync } from './no-async/solution';
import { renderItems as withAsync } from './no-async/solution';

describe('Render Items | Interview | Testcases', () => {
  const testRenderItems = (
    label: string,
    renderItems: (items: Item[], render: RenderFn) => Promise<void>,
  ): void => {
    describe(label, () => {
      beforeEach(() => {
        vi.useFakeTimers();
  ...
  testRenderItems('No Async', noAsync);
  testRenderItems('With Async', withAsync);
});
